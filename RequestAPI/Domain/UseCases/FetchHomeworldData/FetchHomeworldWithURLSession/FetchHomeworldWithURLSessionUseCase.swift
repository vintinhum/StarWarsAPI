...
extension FetchHomeworldWithURLSessionUseCase: FetchHomeworldWithURLSessionUseCaseProtocol {
    ...
    func execute(body: HomeworldRequestModel) -> Observable<UseCaseEventType> {
        request(body: body)
    ...
    private func request(body: HomeworldRequestModel) -> Observable<ServiceResultReturningType> {
        return Observable.create { [weak self] observer in
            let request: Request = .homeworld(request: body)
            self?.service.fetchHomeworld(request: request) { serviceResult in
