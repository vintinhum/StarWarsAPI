...
class CharacterSelectionView: UIView {
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            alamofireRequestButton.centerXAnchor.constraint(equalTo: centerXAnchor),
    ...
    private func validateCharacterNumber(_ characterNumber: Int) -> Bool {
    ...
    @objc private func requestWithURLSession() {
        let requestType: RequestType = .urlsession
        guard let characterNumber = Int(characterNumberTextField.text ?? "0") else { return }
        
        isNumberValid = validateCharacterNumber(characterNumber)
        if isNumberValid {
            delegate?.request(with: requestType, for: characterNumber)
        } else {
            showErrorPopUp()
        }
    }
    ...
    @objc private func requestWithAlamofire() {
        let requestType: RequestType = .alamofire
        guard let characterNumber = Int(characterNumberTextField.text ?? "0") else { return }
